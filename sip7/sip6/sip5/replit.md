...
- The application source code lives in the `sip4/` subdirectory.
- Workflow `Start application` runs `cd sip4 && npm run dev` and serves on port 5000 (frontend + Express backend in one process).
- Vite is configured to bind `0.0.0.0` with `allowedHosts: true` so the Replit iframe proxy can reach it.
- Deployment is set to autoscale: build `cd sip4 && npm run build`, run `cd sip4 && npm start`.
- The app uses `MemStorage` (in-memory) by default; `server/db.ts` is unused, so no database is required.
