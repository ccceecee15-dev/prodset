...
import { ChevronLeft, ChevronRight, ChevronDown, PackagePlus } from "lucide-react";
...
const navGroups: NavGroup[] = [
  {
    ...
    items: [
      ...
      {
        icon: PackagePlus,
        label: "Product Setup Legacy",
        href: "/product-setup-legacy",
      }
    ]
  }
];
...
export function Sidebar() {
  ...
  const [expandedItems, setExpandedItems] = useState<Set<string>>(() => {
    ...
    return new Set(["SIP", "RAP", "Reports"]);
  });
  ...
  const toggleExpanded = (label: string) => {
    ...
    try { localStorage.setItem("sidebar-expanded", JSON.stringify([...newExpanded])); } catch {}
  };
  ...
  return (
    ...
    >
      <div className="h-16 flex items-center justify-between px-4 border-b border-sidebar-border flex-shrink-0">
        ...
        {!collapsed && (
          <button 
            onClick={() => { const v = !collapsed; setCollapsed(v); try { localStorage.setItem("sidebar-collapsed", String(v)); } catch {} }}
